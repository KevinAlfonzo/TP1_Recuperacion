	...
	<div id="layoutSidenav_content">
		<main>
			...
			<div class="container-fluid px-4">
				<h1 class="mt-4">Personas Activas</h1>
				<div class="card-body">
				...
				<div class="card" id="divRegistro" style="display: none;">
					...
					<div class="card-body">
						<form class="row g-3 needs-validation" novalidate>
							...
							<div class="col-md-4">
								<label for="frmCell_phone" class="form-label">N
...
<script>
	...
	// 	let frmDistrict = document.getElementById("frmDistrict");
	...
	function fnBtnBuscar() {
		...
		let url = "PersonBuscar?names=" + names + "&last_names=" + last_names + "&number_document=" + number_document;
	...
	function fnBtnActualizar() {
		let xhttp = new XMLHttpRequest();
		xhttp.open("GET", "PersonActualizar", true);
		xhttp.onreadystatechange = function() {
			if (this.readyState == 4 && this.status == 200) {
				let respuesta = xhttp.responseText;
				arreglo = JSON.parse(respuesta);
...
// 				frmDistrict_document.value = item.district;
...
// 				frmStatus.value = item.status;
	...
	function fnEstadoFormulario(estado){
		frmType_person.disabled = (estado==ACCION_ELIMINAR)
...
<script>
	...
	var celularInput = document.getElementById('frmCell_phone');
...
<!-- VALIDACION CAMPO TIPO DE PERSONA -->
<script>
	// Obtener el campo de entrada de Tipo de Documento
	var tipoPersonaInput = document.getElementById('frmType_person');

	// Obtener el elemento del check
	var tipoPersonaIcon = document.getElementById('frmType_person');

	// Agregar un event listener para el evento 'input'
	tipoPersonaInput.addEventListener('input', function (event) {
		// Obtener el valor actual del campo de Tipo de Documento y eliminar los espacios en blanco
		var tipoPersona = event.target.value.trim();

		// Validar el valor ingresado
		if (tipoPersona === '') {
			// El campo est� vac�o
			tipoPersonaInput.classList.remove('is-valid');
			tipoPersonaInput.classList.remove('is-invalid');
			tipoPersonaIcon.style.display = 'none';
		} else if (/^[A-Z]+$/.test(tipoPersona)) {
			// El valor es v�lido: solo letras en may�scula
			tipoPersonaInput.classList.remove('is-invalid');
			tipoPersonaInput.classList.add('is-valid');
			tipoPersonaIcon.style.display = 'inline-block';
		} else {
			// El valor es inv�lido
			tipoPersonaInput.classList.remove('is-valid');
			tipoPersonaInput.classList.add('is-invalid');
			tipoPersonaIcon.style.display = 'none';
		}
	});
</script>
...
<script>
	function exportToExcel() {
		...
		link.download = 'ListadoPersonas.xlsx';
...
<script>
	function exportToCSV() {
		...
		link.target = '_blank';
		link.download = 'ListadoPersonas.csv';
...
<script>
	let personas = []; // Array para almacenar los datos de las personas
	...
	async function inicializarTabla() {
		personas = await cargarDatosPersonas(); // Carga los datos desde el servidor
	...
	function displayTable() {
		...
		const paginatedItems = personas.slice(startIndex, endIndex);

		// Agrega los registros a la tabla
		paginatedItems.forEach(function (person) {
