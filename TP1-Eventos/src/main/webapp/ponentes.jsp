    ...
    <div id="layoutSidenav_content">
        <main>
            ...
            <div class="container-fluid px-4">
                <h1 class="mt-4">Ponentes Activos</h1>
                <div class="card-body">
                ...
                <div class="card" id="divRegistro" style="display: none;">
                    ...
                    <div class="card-body">
                        <form class="row g-3 needs-validation" novalidate>
                            ...
                            <div class="col-md-4">
                                <label for="frmSpecialty" class="form-label">Especialidad</label>
                                <input type="text" class="form-control" id="frmSpecialty">
                            </div>
                            <div class="col-md-4">
...
<script>
    ...
    let frmSpecialty = document.getElementById("frmSpecialty");
    ...
    function fnBtnProcesar() {
        ...
        datos += "&specialty=" + document.getElementById("frmSpecialty").value;
        datos += "&status=" + document.getElementById("frmStatus").value;
        let xhr = new XMLHttpRequest();
        xhr.open("POST", "PonentesProcesar", true);
    ...
    function fnBtnBuscar() {
        ...
        let url = "PonentesBuscar?names=" + names + "&last_names=" + last_names + "&number_document=" + number_document;
        ...
        xhttp.onreadystatechange = function() {
            if (this.readyState == 4 && this.status == 200) {
                ...
                arreglo.forEach(function(item) {
                    ...
                    detalleTabla += "<td>" + item.specialty + "</td>";
                    detalleTabla += "<td>";
    ...
    function fnBtnActualizar() {
        let xhttp = new XMLHttpRequest();
        xhttp.open("GET", "PonentesActualizar", true);
        xhttp.onreadystatechange = function() {
            if (this.readyState == 4 && this.status == 200) {
                let respuesta = xhttp.responseText;
                arreglo = JSON.parse(respuesta);
    ...
    function fnCargarForm(id){
        arreglo.forEach(function(item) {
            if(item.id == id){
                ...
                frmSpecialty.value = item.specialty;
    ...
    function fnEstadoFormulario(estado){
        ...
        frmSpecialty.disabled = (estado==ACCION_ELIMINAR)
...
<script>
    function exportToExcel() {
        ...
        link.download = 'ListadoPonentes.xlsx';
...
<script>
    function exportToCSV() {
        ...
        link.target = '_blank';
        link.download = 'ListadoPonentes.csv';
...
<script>
    let ponentes = []; // Array para almacenar los datos de las personas
    ...
    async function inicializarTabla() {
        async function cargarDatosPonentes() {
            return [];
        }

        ponentes = await cargarDatosPonentes(); // Carga los datos desde el servidor
        displayTable(); // Muestra la tabla con los primeros registros
    }

    // Función para mostrar la tabla con paginación automática
    function displayTable() {
        const tableBody = document.querySelector("#ponentes-table tbody");

        // Calcula el índice de inicio y fin de los registros a mostrar
        const startIndex = (currentPage - 1) * recordsPerPage;
        const endIndex = startIndex + recordsPerPage;
        const paginatedItems = ponentes.slice(startIndex, endIndex);

        // Agrega los registros a la tabla
        paginatedItems.forEach(function (ponentes) {
            const row = document.createElement("tr");
            row.innerHTML = `
            <td>${ponentes.id}</td>
            <td>${ponentes.names}</td>
            <td>${ponentes.last_names}</td>
            <td>${ponentes.email}</td>
            <td>${ponentes.cell_phone}</td>
            <td>${ponentes.type_document}</td>
            <td>${ponentes.number_document}</td>
            <td>${ponentes.specialty}</td>
            <td>${ponentes.status}</td>
        `;
